...
public class ComparisonHistogram_upperThresh {
    public ComparisonHistogram_upperThresh() {
    }

    public static void main(String[] args) {
        new ComparisonHistogram_upperThresh().createHistogram(args);
    }
    ...
    void createHistogram(String[] args) {
        ...
        try {
            ...
            Vector fileData = FileUtil.readFile(dataFile, 1);
            ...
            double pctDiffThresh = Double.parseDouble(args[3]);
            ...
            for (int dataCtr = 0; dataCtr < fileData.size(); dataCtr++) {
                ...
                //                    histogramBin = 10;
                //                }
                //This is bs, but okay for now
                int histogramBin;
                if (Math.abs(expectedMiPctDiff) < pctDiffThresh && Math.abs(expectedMiPctDiff) > 0.01) {
                    histogramBin = 0;
                } else {
                    histogramBin = 1;
                }
            ...
            //again, BS
            for (int histoCtr = 0; histoCtr < 1; histoCtr++) {
